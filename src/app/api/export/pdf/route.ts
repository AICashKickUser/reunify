...
/**
 * Escape HTML special characters to prevent XSS in the PDF report.
 */
  ...
  try {
    ...
    if (caseData.requirements.length > 0) {
      ...
      caseData.requirements.forEach(r => {
        sections += `<tr><td>${escapeHtml(r.category)}</td><td>${escapeHtml(r.title)}</td><td>${escapeHtml(r.frequency) || 'N/A'}</td><td>${r.isCompleted ? '<span class="completed">Completed</span>' : '<span class="pending">In Progress</span>'}</td><td>${fd(r.completedAt)}</td></tr>`
      })
      sections += `</table>`
    }
    ...
    if (caseData.counselingSessions.length > 0) {
      ...
      caseData.counselingSessions.forEach(s => {
        sections += `<tr><td>${fd(s.date)}</td><td>${escapeHtml(s.counselorName) || 'N/A'}</td><td>${escapeHtml(s.sessionType) || 'N/A'}</td><td>${s.duration ? s.duration + ' min' : 'N/A'}</td><td>${s.isCompleted ? '<span class="completed">Completed</span>' : '<span class="pending">Scheduled</span>'}</td></tr>`
      })
      sections += `</table>`
    }
    ...
    if (caseData.drugTests.length > 0) {
      ...
      caseData.drugTests.forEach(t => {
        ...
        const resultText = t.result === 'negative' ? 'Negative' : t.result === 'positive' ? 'Positive' : escapeHtml(t.result) || 'Pending'
        sections += `<tr><td>${fd(t.date)}</td><td>${escapeHtml(t.testType) || 'N/A'}</td><td>${t.isRandom ? 'Yes' : 'No'}</td><td>${resultClass ? `<span class="${resultClass}">${resultText}</span>` : resultText}</td><td>${escapeHtml(t.testingFacility) || 'N/A'}</td></tr>`
      })
      sections += `</table>`
    }
    ...
    if (caseData.naMeetings.length > 0) {
      ...
      caseData.naMeetings.forEach(m => {
        sections += `<tr><td>${fd(m.date)}</td><td>${escapeHtml(m.meetingName) || 'N/A'}</td><td>${escapeHtml(m.location) || 'N/A'}</td><td>${escapeHtml(m.topic) || 'N/A'}</td><td>${m.isVerified ? '<span class="completed">Verified</span>' : 'Unverified'}</td></tr>`
      })
      sections += `</table>`
    }
    ...
    if (caseData.supervisedVisits.length > 0) {
      ...
      caseData.supervisedVisits.forEach(v => {
        sections += `<tr><td>${fd(v.date)}</td><td>${escapeHtml(v.visitType) || 'N/A'}</td><td>${escapeHtml(v.location) || 'N/A'}</td><td>${escapeHtml(v.supervisorName) || 'N/A'}</td><td>${v.duration ? v.duration + ' min' : 'N/A'}</td><td>${v.isCompleted ? '<span class="completed">Completed</span>' : '<span class="pending">Scheduled</span>'}</td></tr>`
      })
      sections += `</table>`
    }
    ...
    if (caseData.parentingClasses.length > 0) {
      ...
      caseData.parentingClasses.forEach(p => {
        sections += `<tr><td>${fd(p.date)}</td><td>${escapeHtml(p.className) || 'N/A'}</td><td>${escapeHtml(p.provider) || 'N/A'}</td><td>${escapeHtml(p.topic) || 'N/A'}</td><td>${p.isCompleted ? '<span class="completed">Completed</span>' : '<span class="pending">Upcoming</span>'}</td><td>${p.hasCertificate ? 'Yes' : 'No'}</td></tr>`
      })
      sections += `</table>`
    }
