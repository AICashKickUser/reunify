#!/usr/bin/env node
/**
 * Build Reunify TWA v1.1.0 (versionCode: 2) using @bubblewrap/core
 */
...
console.log('=== Building ReunifyMe TWA v1.1.0 ===\n');
...
try {
  execSync(`npx @bubblewrap/cli generate --manifest="${path.join(PROJECT_DIR, 'twa-manifest.json')}" --directory="${PROJECT_DIR}"`, {
    cwd: PROJECT_DIR,
    stdio: 'inherit',
    env: {
      ...process.env,
  ...
  console.log('\nTrying with --skipPwaValidation flag...');
  try {
    execSync(`npx @bubblewrap/cli generate --manifest="${path.join(PROJECT_DIR, 'twa-manifest.json')}" --directory="${PROJECT_DIR}" --skipPwaValidation`, {
      cwd: PROJECT_DIR,
      stdio: 'inherit',
      env: {
        ...process.env,
    ...
    console.log('\nProject generated with skip validation!');
  } catch (error2) {
    console.error('Generation with skip validation also failed:', error2.message);
    process.exit(1);
  }
}
...
const aabSource = path.join(PROJECT_DIR, 'app-release-bundle.aab');
const aabDest = path.join(BUILD_DIR, 'reunify-1.1.0.aab');
const apkSource = path.join(PROJECT_DIR, 'app-release-signed.apk');
const apkDest = path.join(BUILD_DIR, 'reunify-1.1.0.apk');
