#!/usr/bin/env node
/**
 * Build Reunify TWA v1.2.0 (versionCode: 3) using @bubblewrap/core
 * Bug fixes: Pro page mobile, Timeline crash, clickable dashboard, owner bypass
 */
...
const PROJECT_DIR = path.join(BUILD_DIR, 'twa-v1.2.0');
...
console.log('=== Building ReunifyMe TWA v1.2.0 ===\n');
...
try {
  ...
  console.error('Generation failed, trying with auto-yes...');
  // Try with --yes flag or just pipe the answer
  try {
...
const aabDest = path.join(BUILD_DIR, 'reunify-1.2.0.aab');
const apkDest = path.join(BUILD_DIR, 'reunify-1.2.0.apk');
